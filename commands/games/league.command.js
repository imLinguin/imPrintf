    ...
    switch (args[0]) {
      ...
      case "s":
        if (!args[1]) {
          return message.reply(
            `You need to provide your League of Legends nick!`
          );
        }
        const user = await fetch(
          `https://eun1.api.riotgames.com/lol/summoner/v4/summoners/by-name/${args[1]}?api_key=${process.env.riotKey}`
        ).then((res) => {
          return res.json();
        });
        ...
        if (user.status && user.status.status_code !== 200) {
          return message.channel.send(
            `I couldn't find user with this nickname <:Sadge:766682987214667807>`
          );
        }

        const leagueData = await fetch(
          `${leagueDataURL}/${user.id}?api_key=${process.env.riotKey}`
        ).then((res) => {
          return res.json();
        });
        ...
        if (!leagueData[0]) {
          return message.channel.send(
            `**${user.name}**'s level: **${user.summonerLevel}**\nNo rank data found!`,
            new MessageAttachment(
              `http://ddragon.leagueoflegends.com/cdn/${version}/img/profileicon/${user.profileIconId}.png`,
              `logo.png`
            )
          );
        }
        const tierEmote = getTierIcon(leagueData[0].tier);
        const avatar = new MessageAttachment(
          `http://ddragon.leagueoflegends.com/cdn/${version}/img/profileicon/${user.profileIconId}.png`,
          `avatar.png`
        );
        const WinRate =
          (
            leagueData[0].wins /
            (leagueData[0].wins + leagueData[0].losses)
          ).toPrecision(4) * 100;
        const embed = {
          color: 0x095ff,
      ...
      case "r":
        const rotationData = await fetch(
          `https://eun1.api.riotgames.com/lol/platform/v3/champion-rotations?api_key=${process.env.riotKey}`
        ).then((res) => {
          return res.json();
        });
        if (rotationData) {
          ...
          for (var i in rotationData.freeChampionIds) {
            for (var y in championList)
              if (
                championList[y].key.startsWith(rotationData.freeChampionIds[i])
              ) {
          ...
          for (var i in rotationData.freeChampionIdsForNewPlayers) {
            for (var y in championList)
              if (
                championList[y].key.startsWith(
                  rotationData.freeChampionIdsForNewPlayers[i]
                )
              ) {
...
const getTierIcon = (tier) => {
  emotes = [
    `<:SILVER:756983469292126229>`,
    `<:PLATINUM:756983483603353690>`,
    `<:MASTER:756983498509910116>`,
    `<:IRON:756983486434246767>`,
    `<:GRANDMASTER:756983498832871555>`,
    `<:GOLD:756983498312777819>`,
    `<:DIAMOND:756983498937729154>`,
    `<:BRONZE:756983492419518516>`,
    `<:CHALLANGER:756983499637915739>`,
  ];
...
const bestChamp = async (user) => {
  let MysteryData = await fetch(
    `https://eun1.api.riotgames.com/lol/champion-mastery/v4/champion-masteries/by-summoner/${user.id}?api_key=${process.env.riotKey}`
  ).then(async (json) => {
