...
const randomUrl= 'http://api.tenor.com/v1/random?key=V37IT23T7QBQ&q=funny&locale=pl_PL&limit=20';
...
{
    await fetch(trendingUrl).then(res =>{return res.json()}).then(json =>
    {
        try{
...
{
    await fetch(randomUrl).then(res =>{return res.json()}).then(json =>
    {
        try{
        var pom = new MessageAttachment(json.results[Math.floor((Math.random() * 20) + 1)].media[0].tinygif.url);
        msg.channel.send(pom);
...
async function Search(msg,arg)
{
    ...
    await fetch(url).then(res =>{return res.json()}).then(json =>
        {
